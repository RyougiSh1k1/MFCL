...
class CL_dataset():
    def __init__(self, args):
        ...
        # Calculate classes per task based on total classes and number of tasks
        self.n_classes_per_task = len(self.classes) // self.args.n_tasks
        
        # Add samples_per_task parameter
        self.samples_per_task = getattr(args, 'samples_per_task', None)
        ...
        for i, label in enumerate(self.classes):
            ...
            if i % self.n_classes_per_task == (self.n_classes_per_task - 1):
                ...
                if self.samples_per_task is not None:
                    ...
                    if len(current_train) > self.samples_per_task:
                        current_train = np.random.choice(current_train, 
                                                       self.samples_per_task, 
                                                       replace=False).tolist()
                
                self.train_ds += [current_train]
                total_test.extend(current_test)
                self.cl_test_loaders.append(DataLoader(Subset(self.test_dataset, current_test), 
                                                     batch_size=self.args.batch_size, shuffle=False))
                self.full_test_loaders.append(DataLoader(Subset(self.test_dataset, deepcopy(total_test)), 
                                                       batch_size=self.args.batch_size, shuffle=False))
    ...
    def get_task_group(self, task_id, num_users):
        ...
        if self.samples_per_task is not None:
            samples_per_client = self.samples_per_task // num_users
            # Ensure we don't exceed available samples
            samples_per_client = min(samples_per_client, len(train_indx) // num_users)
        else:
            samples_per_client = len(train_indx) // num_users
        
        groups, _ = create_lda_partitions(dataset=targets, num_partitions=num_users, 
                                        concentration=self.args.alpha, accept_imbalanced=False)
        
        # Map back to original indices and limit samples per client
        final_groups = []
        for i in range(num_users):
            client_indices = (np.array(train_indx)[groups[i][0]]).tolist()
            # Limit to samples_per_client
            if len(client_indices) > samples_per_client:
                client_indices = client_indices[:samples_per_client]
            final_groups.append(client_indices)
