...
def main():
    ...
    # Store experiment results for cleanup on error
    max_accuracy = []  # Stores the maximum accuracy achieved for each task
    all_task_accuracies = []  # Stores accuracies for all tasks at each evaluation point
    ...
    try:
        ...
        for t in range(args.n_tasks):
            ...
            try:
                ...
                if t == 0:
                    # For the first task, just evaluate on that task
                    correct, total = evaluate_accuracy(global_model, test_loader, args.method)
                    accuracy = 100 * correct / total
                    max_accuracy.append(accuracy)
                    all_task_accuracies.append([accuracy])
                else:
                    # For subsequent tasks, evaluate on all previous tasks
                    correct, total, accuracies = evaluate_accuracy_forgetting(global_model, dataset.get_cl_test(t), args.method)
                    all_task_accuracies.append(accuracies)
                    
                    # Update max_accuracy for the new task
                    # max_accuracy should have entries for tasks 0 to t-1, now add task t
                    max_accuracy.append(accuracies[-1])
                    
                    # Calculate forgetting: for each previous task, compare its max accuracy with current accuracy
                    forgetting_per_task = []
                    for i in range(t):  # Only calculate forgetting for previous tasks, not the current one
                        forgetting_per_task.append(max_accuracy[i] - accuracies[i])
                    
                    # Average forgetting across all previous tasks
                    avg_forgetting = sum(forgetting_per_task) / len(forgetting_per_task) if forgetting_per_task else 0
                    log_forgetting_results(logger, t, accuracies, avg_forgetting)
        ...
        if len(max_accuracy) > 0 and len(final_accuracies) > 0:
            ...
            total_forgetting = sum([max_accuracy[i] - final_accuracies[i] for i in range(valid_tasks)]) / valid_tasks
            logger.info(f'\nFinal average forgetting: {total_forgetting:.2f}%')

        # Log comprehensive final results
        log_final_results(logger, max_accuracy, final_accuracies, args.n_tasks)

        # Save results to files
        save_results_summary(log_dir, max_accuracy, final_accuracies, args.n_tasks, args.method)
